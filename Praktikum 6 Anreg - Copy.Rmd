...
```{r}
dt <- read_excel("D:\\Documents\\Data 6.xlsx")
dt # menampilkan data
```
...
Hubungan antara peubah IPM dan PPM cenderung tidak linear.
...
Hubungan antara peubah RLS dan PPM cenderung tidak linear
...
Hubungan antara peubah PBH dan PPM tidak linear, ada satu amatan dengan nilai yang jauh dari amatan lainnya
...
Hubungan antara peubah PDRB dan PPM cenderung tidak linear
...
Hubungan antara peubah PKB dan PPM cenderung tidak linear
...
1. RLS dan IPM memiliki korelasi 0.32.\
2. PDRB dengan IPM dan RLS masing-masing sebesar 0.42 dan 0.15.\
3. PKB dengan IPM dan PDRB menunjukkan korelasi negatif, yaitu -0.36 dan -0.24.\
...
$$\hat{PPM}=-0.2061−0.0950IPM−0.7958RLS+0.4191PBH+0.0004PDRB+0.3618PKB$$
...
$$\hat{PPM}=-3.6412−0.1260IPM+0.5749PBH+0.0004PDRB+0.3411PKB$$
...
Nilai p pada uji breusch pagan sama dengan 0.944 yang lebih besar dari alpha 5%, sehingga dinyatakan asumsi ragam sisaan homogen terpenuhi
...
Nilai p pada uji durbin watson sama dengan 0.814 yang lebih besar dari alpha 5%, sehingga dinyatakan asumsi sisaan saling bebas terpenuhi
...
Nilai p pada uji shapiro wilk sama dengan 0.4921 yang lebih besar dari alpha 5%, sehingga dinyatakan asumsi normalitas sisaan terpenuhi
...
Amatan ke-12 terdeteksi sebagai pencilan
...
Tidak terdeteksi titik amatan yang ekstrem

```{r}
ols_plot_resid_lev(model2)
```

Plot yang terbentuk dari fungsi `ols_plot_resid_lev()` Amatan ke-12 adalah outlier murni (pencilan), tetapi bukan leverage point, sehingga belum bisa langsung disebut sebagai titik berpengaruh (influential observation).
...
Amatan ke-12 termasuk amatan berpengaruh sehingga sangat riskan jika disisihkan. Perlu ditelusuri lebih lanjut
...
Berdasarkan hasil perbandingan tersebut, didapatkan bahwa model 3,4,5,6,7,9 yang menyisihkan amatan ke-12 memiliki nilai adjusted R square terbesar dan bukan SSE yang terkecil. 
