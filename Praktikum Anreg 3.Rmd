...
```{r include=FALSE}
library(readxl)
dt <- read_xlsx("D:\\Documents\\PPT kuliah\\smester IV\\Pengaruh pendidikan terhadap upah 2023.xlsx", sheet="Sheet2")
dt
```

```{r}
y <- dt$"Upah minimum (juta)"
x <- dt$"rata-rata lama sekolah"
n <- length(dt)
```
...
Berdasarkan scatter plot, terlihat adanya kecenderungan hubungan linear positif antara variabel x dan y, yang ditunjukkan oleh garis regresi yang meningkat. Namun, titik-titik pengamatan menyebar cukup luas di sekitar garis regresi, sehingga menunjukkan bahwa hubungan antara x dan y relatif lemah. Selain itu, terdapat beberapa amatan yang menyimpang dari pola umum, yang mengindikasikan adanya keragaman galat dalam model. Keragaman dari galat tiap amatan ini dapat diurai berdasarkan garis dugaan persamaan ($\hat{Y}$) dan garis rataan nilai resposnya ($\bar{Y}$). Penguraian keragaman ini dapat dituliskan sebagai Jumlah Kuadrat Regresi (JKR), Jumlah Kuadrat Galat (JKG), dan Jumlah Kuadrat Total (JKT) dengan perhitungan sebagai berikut.
...
Nilai penguraian keragaman ini dapat diamati pula menggunakan fungsi anova pada bagian **Sum Sq (Jumlah Kuadrat)** seperti yang ditampilkan di bawah. Pada output sintaks di bawah, JKR model bernilai 1.4304 dan JKG model bernilai 10.5335

```{r}
(anova.model <- anova(model))
```

Dari tabel ANOVA dan penguraian keragaman, dapat diduga pula ragam galat/error dari model yang kita peroleh. Secara umum, error dari model dapat diduga oleh nilai Kuadrat Tengah Galat (KTG) sehingga berdasarkan output sintaks di atas, diketahui bahwa ragam galat dari model bernilai 0.329. Hubungan antara keduanya dapat dituliskan sebagai berikut.
...
$$S_e = \sqrt{S_{e}^2}$$
Oleh karena simpangan baku merupakan akar kuadrat dari ragam, maka nilai dugaan galat baku model yang kita bentuk adalah $S_e = \sqrt{2.831974} \approx 0.5737349$
...
Sebelumnya, melalui fungsi `lm` dan perhitungan manual yang sudah dibahas pada pertemuan sebelumnya, kita sudah mendapatkan nilai dugaan parameter β0 dan β1 sebesar 0.8850799 dan 0.2283053. Kedua nilai dugaan parameter tersebut dapat kita definisikan sebagai berikut.
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah [-1.689441 ,  3.459601] dan untuk parameter $β_1$ adalah [-0.05323009 ,  0.5098408]
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan rataan nilai $Y$ ketika nilai x=10 adalah 3.168133. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai dugaan rataan $Y$ ketika nilai x=10 berada dalam selang 2.856042 hingga 3.480224.
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu Y ketika nilai x=10 adalah 3.168133. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu Y ketika nilai x=10 berada dalam selang 1.958519 hingga 4.377747.
